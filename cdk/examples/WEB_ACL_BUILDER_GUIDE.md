...
builder.name('MyWebACL').rateLimitConfig({
...
builder.name('MinimalWebACL').noDefaultCustomRules(); // Keep AWS rules, remove rate limiting & geo-blocking
const webAcl = builder.toWebAcl();
```
...
builder.name('ProductionWebACL').managedRuleGroupsMode('active'); // Block instead of count
const webAcl = builder.toWebAcl();
```
